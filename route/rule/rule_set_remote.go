...
func (s *RemoteRuleSet) StartContext(ctx context.Context, startContext *adapter.HTTPStartContext) error {
	...
	if s.lastUpdated.IsZero() {
		...
		if err != nil {
			// DEGRADE, don't brick: a failed initial fetch (no cache yet, e.g.
			// fresh install + the rule-set host unreachable) must NOT fail kernel
			// startup. Previously this returned, taking down the whole service —
			// in production the VPN extension simply would not start. Instead log
			// and continue: the rule-set stays empty (its rules just don't match,
			// like upstream missing_behaviour: default) and PostStart's loopUpdate
			// keeps retrying every update_interval — and because lastUpdated is
			// still zero, loopUpdate retries immediately on start. cache_file
			// covers the normal case after the first success.
