...
type Outbound struct {
	...
	packetAddr      bool
	xudp            bool
	// realm dials a fresh VLESS-Reality-over-realm conn per call. nil unless
	// options.Realm is set; when set, the dialer/tls/transport fields are unused.
	realm *realmDialer
}
...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.VLESSOutboundOptions) (adapter.Outbound, error) {
	...
	if options.Realm != nil {
		// VLESS framing now lives in overlay/reality, so the realm path needs no
		// kernel-side vless.Client — overlay.DialConn writes the request header.
		outbound.realm, err = newRealmDialer(ctx, logger, options)
		if err != nil {
			return nil, err
		}
		return outbound, nil
...
func (h *Outbound) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	if h.realm != nil {
		...
		case N.NetworkTCP:
			h.logger.InfoContext(ctx, "outbound realm connection to ", destination)
			return h.realm.DialContext(ctx, destination)
		default:
			// One punched hole carries one VLESS-Reality stream (WrapStream);
			// UDP-associate is not wired over realm. TCP-tunnelled DNS/HTTP works.
			return nil, E.New("vless-reality-realm: unsupported network: ", network)
		}
	}
...
func (h *Outbound) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	if h.realm != nil {
		return nil, E.New("vless-reality-realm: UDP-associate is not supported over realm")
	}
	if h.multiplexDialer == nil {
		h.logger.InfoContext(ctx, "outbound packet connection to ", destination)
