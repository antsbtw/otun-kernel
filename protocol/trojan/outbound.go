...
type Outbound struct {
	...
	// realm dials a fresh Trojan-over-realm conn per call. nil unless
	// options.Realm is set; when set, all other fields above are unused.
	realm *realmDialer
}
...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.TrojanOutboundOptions) (adapter.Outbound, error) {
	if options.Realm != nil {
		realm, err := newRealmDialer(ctx, logger, options)
		if err != nil {
			return nil, err
		}
		return &Outbound{
...
func (h *Outbound) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	if h.realm != nil {
		...
		case N.NetworkTCP:
			...
			return h.realm.DialConn(ctx, destination)
		default:
			// One punched hole carries one Trojan stream (WrapStream); UDP-associate
			// is not wired over realm. TCP-tunnelled DNS/HTTP still works.
			return nil, E.New("trojan-realm: unsupported network: ", network)
		}
	}
...
func (h *Outbound) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	if h.realm != nil {
		return nil, E.New("trojan-realm: UDP-associate is not supported over realm")
	}
	if h.multiplexDialer == nil {
		h.logger.InfoContext(ctx, "outbound packet connection to ", destination)
...
func (h *Outbound) InterfaceUpdated() {
	if h.realm != nil {
		// Stateless dialer: each live connection owns its own punched hole and is
		// torn down independently. The next DialContext punches fresh, so there is
