...
type Outbound struct {
	...
	// realm dials a fresh SS-over-realm conn per call. nil unless options.Realm
	// is set; when set, all other fields above are unused.
	realm *realmDialer
}
...
func (h *Outbound) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if h.realm != nil {
		...
		default:
			// One punched hole carries one SS stream (WrapStream); UDP-associate is
			// not wired over realm. TCP-tunnelled DNS/HTTP still works.
			return nil, E.New("shadowsocks-realm: unsupported network: ", network)
		}
	}
...
func (h *Outbound) ListenPacket(ctx context.Context, destination M.Socksaddr) (net.PacketConn, error) {
	...
	metadata.Destination = destination
	if h.realm != nil {
		return nil, E.New("shadowsocks-realm: UDP-associate is not supported over realm")
	}
	if h.multiplexDialer == nil {
		if h.uotClient != nil {
